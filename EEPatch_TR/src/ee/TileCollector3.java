	...
	implements ISpecialInventory, ISidedInventory, IEEPowerNet
{
	...
	public void sendAllPackets(int i)//continuation
	{
		int j = 0;
		...
		if (j == 0)
		{
			if (collectorSunTime <= 0x493e00 - i)
				collectorSunTime += i;
			return;
		}
		int k = i / j;
	...
	{//start of error
		...
		if (!world.isStatic)
		{
			...
			} else
			{
				...
				sendAllPackets(getFactoredProduction());//error continues here
			}
		}
	}
	...
	{
		return (collectorSunTime * i) / 0x493e00;
	}
