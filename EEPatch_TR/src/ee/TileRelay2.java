...
// Source File Name:   TileRelay2.java
...
public class TileRelay2 extends TileEE implements ISpecialInventory, ISidedInventory, IEEPowerNet {
	...
	public boolean passEnergy(int var1, byte var2, boolean var3) {
		TileEntity var4 = world.getTileEntity(x + (var2 != 5 ? ((int) (var2 != 4 ? 0 : 1)) : -1), y + (var2 != 1 ? ((int) (var2 != 0 ? 0 : 1)) : -1),
				z + (var2 != 3 ? ((int) (var2 != 2 ? 0 : 1)) : -1));
	...
	public boolean sendEnergy(int var1, byte var2, boolean var3) {
		TileEntity var4 = world.getTileEntity(x + (var2 != 5 ? ((int) (var2 != 4 ? 0 : 1)) : -1), y + (var2 != 1 ? ((int) (var2 != 0 ? 0 : 1)) : -1),
				z + (var2 != 3 ? ((int) (var2 != 2 ? 0 : 1)) : -1));
		return var4 != null ? (var4 instanceof TileRelay) || (var4 instanceof TileRelay2) || (var4 instanceof TileRelay3) ? false
				: (var4 instanceof IEEPowerNet) && ((IEEPowerNet) var4).receiveEnergy(var1, var2, var3) : false;
	}
			...
			if (!world.isStatic) {
				...
				if (items[0] != null && EEBase.isKleinStar(items[0])) {
					for (int var2 = getRelayProductivity() * ItemKleinStar.getLevel_s(items[0]); var2 > 0; var2--)
						if (latentEnergy() < relayMaximum() && EEBase.takeKleinStarPoints(items[0], 1, world)) scaledEnergy += 80;
				...
				if (arrayCounter > 0) arrayCounter--;
			}
	...
	private int getItemBurnTime(ItemStack var1) {
		...
		if (EEBase.isKleinStar(var1)) return 0;
	...
	public int getStartInventorySide(int var1) {
		return var1 != 1 ? 0 : items.length - 1;
	}

	public int getSizeInventorySide(int var1) {
		return var1 != 1 ? items.length - 1 : 1;
	}

	public boolean onBlockActivated(EntityHuman var1) {
		if (!world.isStatic) var1.openGui(mod_EE.getInstance(), GuiIds.RELAY_2, world, x, y, z);
		return true;
	}

	public int getTextureForSide(int var1) {
		if (var1 == 1) {
			return EEBase.relay2Top;
		} else {
			byte var2 = direction;
			return var1 != var2 ? EEBase.relaySide : EEBase.relayFront;
		}
	}

	public int getInventoryTexture(int var1) {
		return var1 != 1 ? var1 != 3 ? EEBase.relaySide : EEBase.relayFront : EEBase.relay2Top;
	}
