...
// Source File Name:   TileNovaCatalyst.java
...
//			TileEE, TileAlchChest, EEBase, EntityNovaPrimed, 
...
public class TileNovaCatalyst extends TileEE {
	...
	public int getTextureForSide(int var1) {
		return var1 != 0 ? var1 != 1 ? EEBase.novaCatalystSide : EEBase.novaCatalystTop : EEBase.novaCatalystBottom;
	}
	...
	public void onBlockAdded() {
		super.onBlockAdded();
		if (world.isBlockIndirectlyPowered(x, y, z)) lightFuse();
	}

	public void onNeighborBlockChange(int var1) {
		if (var1 > 0 && Block.byId[var1].isPowerSource() && world.isBlockIndirectlyPowered(x, y, z)) lightFuse();
	}

	public void onBlockClicked(EntityHuman var1) {
		if (var1.U() != null && var1.U().id == Item.FLINT_AND_STEEL.id) lightFuse();
	...
	public void onBlockDestroyedByExplosion() {
		...
		if (player != null && player != "" && (human = world.a(player)) != null) 
			var1 = new EntityNovaPrimed(world, human, x + 0.5F, y + 0.5F, z + 0.5F);
		else
			var1 = new EntityNovaPrimed(world, x + 0.5F, y + 0.5F, z + 0.5F);
		var1.fuse = 0;
		world.addEntity(var1);
	}

	public void onBlockDestroyedByPlayer() {
		if (!world.isStatic) if (!fuseLit) {
