...
// Source File Name:   TileRelay3.java
...
public class TileRelay3 extends TileEE implements ISpecialInventory, ISidedInventory, IEEPowerNet {
	...
	private boolean canDestroy() {
		...
		return items[0] != null ? EEBase.isKleinStar(items[0]) ? true : EEMaps.getEMC(items[0]) != 0 ? scaledEnergy / 80 + EEMaps.getEMC(items[0]) <= relayMaximum() : false : false;
	}
	...
	public boolean onBlockActivated(EntityHuman var1) {
		if (!world.isStatic) var1.openGui(mod_EE.getInstance(), GuiIds.RELAY_3, world, x, y, z);
		return true;
	}

	public int getTextureForSide(int var1) {
		if (var1 == 1) {
			return EEBase.relay3Top;
		} else {
			byte var2 = direction;
	...
	public int getInventoryTexture(int var1) {
		return var1 != 1 ? var1 != 3 ? EEBase.relaySide : EEBase.relayFront : EEBase.relay3Top;
	}
