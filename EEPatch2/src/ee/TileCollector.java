    ...
    implements ISpecialInventory, ISidedInventory, IEEPowerNet
{
    ...
    public ItemStack extractItem(boolean var1, Orientations var2)
    {
        ...
        case 7: // '\007'
            for(int var3 = 0; var3 < items.length; var3++)
                if(items[var3] != null && var3 != items.length - 1)
                    if(var3 == 0)
                    {
                    ...
                    if(items[var3].id == EEItem.aeternalisFuel.id || items[items.length - 1] != null && items[var3].doMaterialsMatch(items[items.length - 1]))
                    {
                        ItemStack var4 = items[var3].cloneItemStack();
                        var4.count = 1;
                        if(var1)
    ...
    {
        return canUpgrade() ? getFuelDifference() > 0 ? (collectorSunTime * var1) / (getFuelDifference() * 80) <= 24 ? (collectorSunTime * var1) / (getFuelDifference() * 80) : 24 : items[0] == null || !EEBase.isKleinStar(items[0].id) ? 0 : 24 : 0;
    }
    ...
    public boolean canUpgrade()
    {
        if(items[0] == null)
        {
            ...
            {
                if(items[var1] == null || items[items.length - 1] != null && items[var1].doMaterialsMatch(items[items.length - 1]) || !EEMaps.isFuel(items[var1]) || items[var1].getItem().id == EEItem.aeternalisFuel.id)
                    continue;
    ...
    {
        TileEntity var4 = world.getTileEntity(x + (var2 != 5 ? ((int) (var2 != 4 ? 0 : 1)) : -1), y + (var2 != 1 ? ((int) (var2 != 0 ? 0 : 1)) : -1), z + (var2 != 3 ? ((int) (var2 != 2 ? 0 : 1)) : -1));
    ...
    public void sendAllPackets(int var1)
    {
        ...
        if(var2 == 0)
        {
            if(collectorSunTime <= 800000 - var1)
                collectorSunTime += var1;
        } else
        {
            int var5 = var1 / var2;
            if(var5 >= 1)
    ...
    public void q_()
    {
        if(!clientFail() && !world.isStatic)
        {
            ...
            if(isUsingPower())
            {
                ...
                if(EEBase.isKleinStar(items[0].id))
                {
                    for(int var1 = getFactoredProduction() * EEBase.getKleinLevel(items[0].id); var1 > 0 && collectorSunTime >= 80 && EEBase.addKleinStarPoints(items[0], 1, world); var1--)
                        collectorSunTime -= 80;
    ...
    {
        return (collectorSunTime * var1) / 800000;
    }

    private int getKleinProgressScaled(int var1)
    {
        return items[0] == null || !(items[0].getItem() instanceof ItemKleinStar) ? 0 : (((ItemKleinStar)items[0].getItem()).getKleinPoints(items[0]) * var1) / ((ItemKleinStar)items[0].getItem()).getMaxPoints(items[0]);
    }
    ...
    private boolean canCollect()
    {
        if(items[0] == null)
        {
            for(int var1 = 1; var1 <= items.length - 3; var1++)
            {
                if(items[var1] == null || items[items.length - 1] != null && (items[items.length - 1] == null || !items[items.length - 1].doMaterialsMatch(items[var1])))
                    continue;
                items[0] = items[var1].cloneItemStack();
                items[var1] = null;
                break;
        ...
        if(items[items.length - 2] != null && !items[items.length - 2].doMaterialsMatch(var3))
            return false;
        ...
        for(int var2 = 1; var2 <= items.length - 2; var2++)
            if(items[var2] != null && (items[var2].getItem().id == EEItem.mobiusFuel.id || items[items.length - 1] != null && items[var2].doMaterialsMatch(items[items.length - 1])) && items[var2].count >= items[var2].getMaxStackSize() && tryDropInChest(new ItemStack(items[var2].getItem(), items[var2].count)))
                items[var2] = null;
    ...
    public void uptierFuel()
    {
        if(canCollect() && getNextFuel(items[0]) != null)
        {
            ...
            if(items[items.length - 2] == null)
            {
                if((items[items.length - 1] == null || !var1.doMaterialsMatch(items[items.length - 1])) && var1.getItem() != EEItem.aeternalisFuel)
                    items[items.length - 2] = var1.cloneItemStack();
                else
                if(!tryDropInChest(var1))
                    items[items.length - 2] = var1.cloneItemStack();
            } else
            if(items[items.length - 2].id == var1.id)
            {
                ...
                if((items[items.length - 1] == null || !var1.doMaterialsMatch(items[items.length - 1])) && var1.getItem() != EEItem.aeternalisFuel)
                    items[items.length - 2].count += var1.count;
                else
                if(!tryDropInChest(var1))
                    items[items.length - 2].count += var1.count;
            } else
            if((items[items.length - 1] != null && var1.doMaterialsMatch(items[items.length - 1]) || var1.getItem() == EEItem.aeternalisFuel) && tryDropInChest(items[items.length - 2].cloneItemStack()))
                items[items.length - 2] = null;
    ...
    {
        if(!world.isStatic)
            var1.openGui(mod_EE.getInstance(), GuiIds.COLLECTOR_1, world, x, y, z);
        return true;
    }
