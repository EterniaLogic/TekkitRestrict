...
public class Updater {
				...
				if (read()) {
					if (versionCheck(Updater.this.versionName)) {
						if ((Updater.this.versionLink != null) && (Updater.this.type != UpdateType.NO_DOWNLOAD)) {
							...
							saveFile(new File(Updater.this.plugin.getDataFolder().getParent(), Updater.this.updateFolder), name, Updater.this.versionLink);
						} else {
	...
	private boolean versionCheck(String title) {
		TRVersion ver2;
		try {
			ver2 = new TRVersion(title);
		} catch (Exception ex){
			this.plugin.getLogger().warning("The author of this plugin (Taeir) has misconfigured his Auto Update system");
			this.plugin.getLogger().warning("File versions should follow the format 'PluginName VERSION Dev/Beta/Release ExtraVersion'");
