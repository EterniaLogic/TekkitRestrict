...
public class TRSafeZone_old {
	...
	public static List<TRSafeZone_old> zones = Collections.synchronizedList(new LinkedList<TRSafeZone_old>());
	...
	public static void init() {
		...
		try {
			...
			while (rs.next()) {
				TRSafeZone_old sz = new TRSafeZone_old();
	 ...
	 */
	public static boolean removeSafeZone(TRSafeZone_old zone){
		if (zone.mode == 1){
	...
	public static SafeZoneCreate addSafeZone(Player player, String pluginName, String name){
		...
		for (TRSafeZone_old current : TRSafeZone_old.zones){
		...
		if (SafeZones.UseGP && pluginName.equals("griefprevention") && PM.isPluginEnabled("GriefPrevention")) {
			...
			TRSafeZone_old zone = new TRSafeZone_old();
			zone.mode = 4;
	...
	public static boolean isGriefPreventionSafeZoneFor(Player p){
		...
		if (SafeZones.GPMode == SSMode.SpecificAdmin){ //SpecificAdmin (first part)
			...
			Iterator<TRSafeZone_old> zonesIterator = zones.iterator();
			while (zonesIterator.hasNext()) {
				TRSafeZone_old a = zonesIterator.next();
	...
	/*public static void setFly(PlayerMoveEvent e) {
		if (SafeZones.SSDisableFly) {
			...
			if (inSafeZone(player)) {
				...
				player.sendMessage(ChatColor.RED + "[TRSafeZone_old] You may not fly in safezones!");
			}
		}
	}*/
	public static class PS {
		 ...
		 */
		public static Field getSafeZoneAt(Location loc){
			//TODO Check if Factions has an option to add flags.
			if (!SafeZones.UseSafeZones || !SafeZones.UsePS || !PM().isPluginEnabled("PreciousStones")) return null;
		...
		/**
		 * Note: All PS zones are safezones by default.<br>
		 * If strict is <b>true</b> then this will check if the player can build/break at his location.
		 * If strict is <b>false</b>, then this will check if the player is allowed at his location.<br>
		 * @return If the PS safezone at &lt;player&gt;'s location applies for him.<br>
		 * If there is no PS safezone at his location, this will return false.
		 */
		public static boolean doesSafeZoneApplyForPlayer(Player p, boolean strict){
			//TODO Check if Factions has an option to add flags.
			if (!SafeZones.UseSafeZones || !SafeZones.UsePS || !PM().isPluginEnabled("PreciousStones")) return false;
			PreciousStones ps = (PreciousStones) PM().getPlugin("PreciousStones");
			
			Field field = getSafeZoneAt(p.getLocation());
			if (field == null) return false;
			boolean allowed = true;
			if (strict){
				allowed = ps.getForceFieldManager().isApplyToAllowed(field, p.getName());
			} else {
				allowed = ps.getForceFieldManager().isAllowed(field, p.getName());
			}
