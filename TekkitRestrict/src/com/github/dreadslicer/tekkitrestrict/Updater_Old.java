...
{	
	...
	private Updater_Old.UpdateResult result = Updater_Old.UpdateResult.SUCCESS; // Used for determining the outcome of the update process
	 ...
	 */
	public Updater_Old(Plugin plugin, String slug, File file, UpdateType type, boolean announce)
	{
		this.plugin = plugin;
		this.type = type;
		...
		catch (MalformedURLException ex)
		{
			...
			result = Updater_Old.UpdateResult.FAIL_BADSLUG; // Bad slug! Bad!
		}
	...
	public Updater_Old.UpdateResult getResult()
	{
		waitForThread();
		return result;
	}
	...
	private void saveFile(File folder, String file, String u)
	{
		...
		try
		{
			...
			while ((count = in.read(data, 0, BYTE_SIZE)) != -1)
			{
				...
				if(announce & (percent % 10 == 0) && lastPercentLog != percent)
				{
					lastPercentLog = percent;
					plugin.getLogger().info("Downloading update: " + percent + "% of " + fileLength/1024 + " kB (KiB).");
				}
			}
		...
		catch (Exception ex)
		{
			...
			result = Updater_Old.UpdateResult.FAIL_DOWNLOAD;
		}
		finally
		{
			try
	...
	private String getFile(String link)
	{
		...
		catch (Exception ex)
		{
			...
			result = Updater_Old.UpdateResult.FAIL_DBO;
			return null;
		}
		return download;
	}
	...
	{
		if(type == UpdateType.NO_VERSION_CHECK) return true;
		...
		if (fullTitle.length >= 3){
			...
			//fullTitle[2] = Beta
			//fullTitle[3] = 2
			int curVerMajor = -1, curVerMinor = -1, curVerSec = -1;
			int remVerMajor = -1, remVerMinor = -1, remVerSec = -1;
			
			if (tekkitrestrict.getMajorVersion().matches("\\d+")) curVerMajor = Integer.parseInt(tekkitrestrict.getMajorVersion());
			if (tekkitrestrict.getMinorVersion().matches("\\d+")) curVerMinor = Integer.parseInt(tekkitrestrict.getMinorVersion());
			
			String temp = tekkitrestrict.getExtraVersion().replaceAll("\\D+", "");//2
			
			if (tekkitrestrict.getExtraVersion().equals("")) curVerSec = 0;
			else if (temp.equals("")) curVerSec = 1;
			else if (temp.matches("\\d+")) curVerSec = Integer.parseInt(temp);
			
			String newver = fullTitle[1];//1.15
			
			if (newver.matches("\\d+\\.\\d+")){
				remVerMajor = Integer.parseInt(newver.split("\\.")[0]);
				remVerMinor = Integer.parseInt(newver.split("\\.")[1]);
			} else if (newver.matches("\\d+")) {
				remVerMajor = Integer.parseInt(newver);
				remVerMinor = 0;
			}
			
			String secTitle = fullTitle[2].toLowerCase();
			
			if (secTitle.contains("beta")){
				if (!tekkitrestrict.isBeta() && !tekkitrestrict.config.getBoolean("UpdateToBetaVersions", false)){
					result = Updater_Old.UpdateResult.NO_UPDATE;
					return false;
				}
				
				newver += "b";
				if (fullTitle.length >= 4 && fullTitle[3].matches("\\d+")){//1.15b3
					newver += fullTitle[3];
					remVerSec = Integer.parseInt(fullTitle[3]);
				}
				
			} else if (secTitle.contains("dev")){
				if (!tekkitrestrict.isDev() && !tekkitrestrict.config.getBoolean("UpdateToDevelopmentVersions", false)){
					result = Updater_Old.UpdateResult.NO_UPDATE;
					return false;
				}
				
				newver += "d";
				if (fullTitle.length >= 4 && fullTitle[3].matches("\\d+")){
					newver += fullTitle[3];
					remVerSec = Integer.parseInt(fullTitle[3]);
				}
			} else if (secTitle.contains("release")){}
			
			if (tekkitrestrict.getFullVersion().equalsIgnoreCase(newver)){
				result = Updater_Old.UpdateResult.NO_UPDATE;
				return false;
			} else if (curVerMajor > remVerMajor || (curVerMajor == remVerMajor && (curVerMinor > remVerMinor) || (curVerMinor == remVerMinor && curVerSec >= remVerSec))){
				result = Updater_Old.UpdateResult.NO_UPDATE;
				return false;
			}
		} else {
			...
			result = Updater_Old.UpdateResult.FAIL_NOVERSION;
			return false;
		}
		return true;
	}
