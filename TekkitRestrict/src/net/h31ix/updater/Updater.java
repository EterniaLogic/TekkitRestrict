    ...
    private boolean versionCheck(String title)
    {
        if(type != UpdateType.NO_VERSION_CHECK)
        {
            ...
            if (fullTitle.length >= 3){
            	...
            	if(((secTitle.contains("beta") || secTitle.contains("dev")) && !tekkitrestrict.config.getBoolean("UpdateToDevelopmentVersions", false)) ||
            		 hasTag(version) || version.equalsIgnoreCase(fullTitle[1]) || curVer >= remVer)
                {
